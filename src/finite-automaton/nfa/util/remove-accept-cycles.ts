...
export function removeAcceptCycles (
    ...
    let openAcceptStates = [...nfa.acceptStates];
    ...
    while (openAcceptStates.length > 0) {
        ...
        const acceptCycle = tryGetAcceptCycle(curAcceptState, nfa.acceptStates, transitions);
        ...
        const keep = (
            acceptCycle.some(t => t.srcState == nfa.startState) ?
            nfa.startState :
        ...
        for (const accept of acceptCycle) {
            ...
            transitions = transitions
                ...
                .map(t => {
                    return {
                        srcState : t.srcState,
                        dstStateSets : t.dstStateSets.map(dstStateSet => {
