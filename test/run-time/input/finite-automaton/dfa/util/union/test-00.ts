...
tape(__filename, (t) => {
    for (const { dfa : dfa1, language10 : language10_1 } of language10Collection) {
        for (const { dfa : dfa2, language10 : language10_2 } of language10Collection) {
            const dfa3 = DfaUtil.union(dfa1, dfa2);
            ...
            t.true(
                ...
                `${dfa1.name} \\cup ${dfa2.name} has extra values ${findExtraValues(language10_3, language10_1_2).join(",")}`
            );
            t.true(
                isSubSet(language10_1_2, language10_3),
                `${dfa1.name} \\cup ${dfa2.name} has missing values ${findExtraValues(language10_1_2, language10_3).join(",")}`
            );
        }
    }
