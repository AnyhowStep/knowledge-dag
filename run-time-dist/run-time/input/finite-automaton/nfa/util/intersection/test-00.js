...
tape(__filename, (t) => {
    for (const { nfa: nfa1, language10: language10_1 } of nfa_collection_1.language10Collection) {
        for (const { nfa: nfa2, language10: language10_2 } of nfa_collection_1.language10Collection) {
            const nfa3 = finite_automaton_1.NfaUtil.intersection(nfa1, nfa2);
            ...
            t.true(nfa_collection_1.isSubSet(language10_3, language10_1_2), `${nfa1.name} \\cap ${nfa2.name} has extra values ${nfa_collection_1.findExtraValues(language10_3, language10_1_2).join(",")}`);
            t.true(nfa_collection_1.isSubSet(language10_1_2, language10_3), `${nfa1.name} \\cap ${nfa2.name} has missing values ${nfa_collection_1.findExtraValues(language10_1_2, language10_3).join(",")}`);
        }
    }
    t.end();
});
