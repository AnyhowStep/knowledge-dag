...
tape(__filename, (t) => {
    for (const { dfa: dfa1, language10: language10_1 } of dfa_collection_1.language10Collection) {
        for (const { dfa: dfa2, language10: language10_2 } of dfa_collection_1.language10Collection) {
            const dfa3 = finite_automaton_1.DfaUtil.union(dfa1, dfa2);
            ...
            t.true(dfa_collection_1.isSubSet(language10_3, language10_1_2), `${dfa1.name} \\cup ${dfa2.name} has extra values ${dfa_collection_1.findExtraValues(language10_3, language10_1_2).join(",")}`);
            t.true(dfa_collection_1.isSubSet(language10_1_2, language10_3), `${dfa1.name} \\cup ${dfa2.name} has missing values ${dfa_collection_1.findExtraValues(language10_1_2, language10_3).join(",")}`);
        }
    }
    t.end();
});
